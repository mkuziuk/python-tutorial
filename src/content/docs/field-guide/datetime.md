...
`datetime` представляет дату и время вместе. Для расследования недостаточно записи `23:07`: без даты и часового пояса два события нельзя надежно сравнить.
...
- `datetime.now()` без аргумента создает локальное наивное время.
- Не удаляйте исходную строку и источник события, если отчет должен сохранять происхождение улики.
