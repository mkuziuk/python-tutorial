...
Используйте эти модули, когда формат уже известен. Они надёжнее, чем ручной разбор строк через срезы.
...
Для письма используйте методы `EmailMessage`, а не ручной обход всех MIME-контейнеров:
...
`get_body()` выбирает нужную текстовую часть, а `iter_attachments()` возвращает вложения.
