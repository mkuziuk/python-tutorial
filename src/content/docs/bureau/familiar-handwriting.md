...
Только после этого тетрадь предлагает исправить разбиение. Заглушка запускается, но лишь добавленный вами `stratify=y` сохранит доли десяти классов осознанно:
...
Сетка намеренно мала и объявлена заранее. В учебной версии временная SVM позволяет продолжить **Run All**, но параметры блокировки останутся `TODO`, пока вы не реализуете `GridSearchCV` только на обучающей выборке:
...
Галерея берёт первые воспроизводимые ошибки тестовой выборки, а не вручную отобранные удачные или эффектные примеры:
...
| Фиксация | Выбранная конфигурация записана до знакомства с партией C финального дела. |
...
После своей версии откройте [дебриф](../familiar-handwriting-solution/). Финальное дело проверит, что случится, если связанные варианты одной цифры разнести между обучающей и тестовой выборками.
