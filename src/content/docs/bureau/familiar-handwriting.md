...
Learner notebook начинает с табличного представления Digits. Сначала он проверяет локальный CSV, создаёт `DataFrame`, а затем явно преобразует 64 пиксельных столбца в массивы модели:
...
Только после этого notebook просит исправить разбиение. Заглушка запускается, но лишь добавленный вами `stratify=y` сохранит доли десяти классов осознанно:
