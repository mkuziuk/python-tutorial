...
<div class="bureau-brief">
  ...
  <p><strong>Данные</strong> тот же замороженный Titanic; корректный конвейер из II-02 уже включён в самостоятельный архив.</p>
...
После подготовки разбиения внешняя тестовая выборка остаётся закрытой, пока одинаковые части выборки сравнивают кандидатов на внешней обучающей выборке. Следующая ячейка собирает несколько метрик, чтобы accuracy не стала единственным критерием:
...
# Порог выбирается только по OOF-прогнозам; тестовая выборка здесь недоступна.
