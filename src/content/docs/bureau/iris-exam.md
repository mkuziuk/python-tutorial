...
Первый листинг получает карточку источника и путь к локальному CSV. Проверка SHA-256 связывает дальнейшие результаты с конкретными байтами файла. `frame` имеет структуру «одна строка на цветок», `feature_names` хранит четыре названия признаков, а `species_by_code` переводит числовой класс в название вида.
...
После выполнения проверьте не только первые строки, но и форму, типы, пропуски, полные дубликаты, числовые диапазоны, уникальные значения и распределение цели. В поставляемом снимке четыре признака числовые, полных дубликатов и пропусков нет, а три класса представлены равными долями. Это делает Iris удобным учебным набором, но не моделирует грязные производственные данные.

Pairplot в тетради отвечает на отдельный вопрос: какие измерения визуально разделяют виды? Setosa образует отдельную область прежде всего по лепестковым признакам, а versicolor и virginica частично перекрываются. График использует уже известный класс, поэтому это разведка, а не оценка модели.

Теперь можно зафиксировать стратифицированное разбиение. Оно получает `X` формы `(число цветков, 4)` и `y` с одним кодом на цветок; на выходе получаются четыре объекта train/test:
...
Близкие доли в двух колонках `split_summary` подтверждают стратификацию. Индексы train и test не должны пересекаться. После этого test остаётся закрытым: `k` выбирается только по пятичастной кросс-валидации train.

`DummyClassifier(strategy="most_frequent")` всегда предсказывает самый частый класс и задаёт baseline. Accuracy — доля верных прогнозов среди всех прогнозов; она лежит от 0 до 1. Сначала правило baseline фиксируется только по train, без вычисления test-метрики:
...
Затем `selected_k` выбирается по средней accuracy внутренней кросс-валидации обучающей выборки. Только после фиксации обоих решений единая сравнительная ячейка открывает test. `StandardScaler` и k-NN объединены в `Pipeline`, чтобы параметры масштабирования также обучались только на train. `StandardScaler` и k-NN объединены в `Pipeline`, чтобы параметры масштабирования также обучались только на train.
...
Первые две строки `comparison` сопоставимы, потому что используют один test. Строка `vendor / train` описывает другую границу: `k=1` оценён на тех же объектах, на которых обучался. Поэтому 100% воспроизводятся, но не являются внешней проверкой.

Матрица ошибок в тетради разворачивает итоговую accuracy в таблицу `3 × 3`: строки обозначают истинные виды, столбцы — прогнозы, диагональ — верные ответы. Внедиагональные ячейки показывают направление путаницы. Реестр ошибок затем возвращает исходные четыре измерения, истинный вид и прогноз для каждого промаха; на этом снимке спорная область связана с близкими versicolor и virginica. Это наблюдение на малом test, а не оценка природной частоты таких ошибок.
