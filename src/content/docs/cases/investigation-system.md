...
В первых четырех делах команда собрала много отдельных материалов: анонимную записку, похожие фрагменты, письма, папку после ночного сигнала, логи и заметки. Отдельные скрипты помогли ответить на конкретные вопросы, но теперь расследование нужно держать целиком.
...
Так можно жить в маленьком скрипте, но доска расследования просит другой формы. Улика должна сама уметь отвечать, подходит ли она под поисковый запрос. Дело должно уметь добавлять заметку, находить улику по ID и сохранять себя обратно в JSON.
