...
Объекты полезны, когда у данных появляется собственное поведение. Улика — это не только набор полей: она должна проверять надёжность, отвечать на поисковый запрос и превращаться обратно в JSON.
...
Словарь остаётся удобным форматом для чтения файла, но внутри программы понятнее работать с объектами: `Evidence` хранит поля одной улики вместе с проверками и поиском, `Investigation` управляет всей коллекцией, а `CaseRepository` отвечает за JSON.
