...
В первых четырёх делах команда собрала много отдельных материалов: анонимную записку, похожие отчёты, письма, папку после ночного сигнала, журналы и заметки. Теперь программа должна загрузить участников, улики и заметки из `case_seed.json`, выполнить поиск, добавить заметку и сохранить результат в `case_report.json`.
...
`add_evidence()` вызывает `ValueError`, если такой `evidence_id` уже существует, и добавляет объект только после этой проверки.
