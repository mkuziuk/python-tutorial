...
Формула использует начальные учебные веса. Вклад каждого признака виден в коде, поэтому веса можно менять и наблюдать, как меняется рейтинг. Итоговая оценка измеряет сходство выбранных признаков; для оценки вероятности авторства потребовалась бы отдельная статистическая модель.
