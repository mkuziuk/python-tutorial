...
Эти веса не священные. Это первая рабочая версия прибора: вклад каждого признака виден прямо в формуле, поэтому ученик может менять числа и наблюдать, как меняется рейтинг. Итоговая оценка показывает сходство выбранных признаков, но не является вероятностью авторства.
