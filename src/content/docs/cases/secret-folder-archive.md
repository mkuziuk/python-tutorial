...
description: "Собираем индексатор папки после ночного сигнала архива: pathlib, SHA-256, JSON-манифест, timestamps и поиск изменений."
concepts:
  - pathlib
  - file traversal
  - hashlib
...
<div class="case-meta">
  <p><strong>Миссия</strong> проверить папку после ночного сигнала: что внутри, какие файлы совпадают и что изменилось с прошлого запуска.</p>
...
После текстов и писем расследование упирается в папку `secret_folder`. Ночью система архива отметила открытие комнаты хранения, а утром в папке лежат фотоиндексы, заметки, служебный лог и две версии хронологии. На глаз все выглядит почти спокойно, но по именам уже непонятно, какие файлы совпадают и что менялось между проверками.

Вопрос дела: как сделать опись папки так, чтобы завтра проверить дубли и изменения, не полагаясь на память и догадки?

Обычная дата изменения помогает, но ей нельзя полностью доверять. Файл можно скопировать, переименовать или сохранить заново. Поэтому мы будем считать криптографический хэш SHA-256 через [`hashlib`](../../field-guide/hashlib/) и сохранять [JSON](../../field-guide/json/)-manifest.
