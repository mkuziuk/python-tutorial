...
`file_sha256()` читает файл блоками, поэтому объём используемой памяти не растёт вместе с размером файла. Функция принимает путь и возвращает строку SHA-256, по которой программа сравнивает содержимое файлов.
...
`main()` сохраняет текущий манифест в `manifest.json`. При следующем запуске `compare_manifests()` сравнит его с новым состоянием папки.
