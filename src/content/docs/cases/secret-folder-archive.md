...
После текстов и писем расследование упирается в папку `secret_folder`. Ночью учебная система отметила открытие комнаты хранения, а утром в папке лежат фотоиндексы, заметки, служебный лог и две версии хронологии. На глаз все выглядит почти спокойно, но по именам уже непонятно, какие файлы совпадают и что менялось между проверками.
