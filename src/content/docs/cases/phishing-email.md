...
description: "Пошагово строим анализатор безопасных .eml-писем: email, regex, urllib.parse, ipaddress, исключения и правила риска."
concepts:
  - regex review
  - email
  - urllib.parse
...
<div class="case-meta">
  <p><strong>Миссия</strong> проверить набор учебных писем и объяснить, какие признаки риска в них видны.</p>
...
Пока команда разбирает текстовые следы, в учебный почтовый ящик приходит письмо про доступ к архивным материалам. Всего в наборе четыре письма `.eml`. Они безопасные и полностью синтетические: домены вымышленные, IP-адреса взяты из документационных диапазонов, реальных брендов и настоящих учетных записей нет.

Вопрос дела: какие признаки риска видны, если не открывать ссылки, не запускать вложения и не доверять письму на слово?
...
- `data/*.eml` - безопасные учебные письма;
...
Мы построим не магическую модель, а маленький набор правил. Это важнее для учебного дела: каждое решение можно объяснить.
...
Это не промышленный антифишинговый сканер. Но это хороший первый прибор: он не пугает одним большим словом "опасно", а показывает, какие именно факты стоит проверить вручную.
...
Здесь появляется первая защитная привычка: код явно говорит, какие части письма он анализирует, а какие оставляет за пределами задачи. Функция возвращает одну строку: это упрощает поиск ссылок и срочных слов, но не делает вид, что мы безопасно открывали вложения.
...
Баллы превращаются в человеческий вердикт. Границы выбраны так, чтобы один слабый сигнал не делал письмо "опасным", а несколько независимых признаков поднимали его наверх отчета:
...
```text
02-account-review.eml  высокий риск       12
04-calendar-mix.eml    проверить вручную   3
01-team-update.eml     низкий риск         0
03-invoice-note.eml    низкий риск         0
```

Если `02-account-review.eml` оказался самым рискованным, а `01-team-update.eml` и `03-invoice-note.eml` остались спокойными, инструмент работает как задумано. Если числа отличаются, проверьте веса правил и то, не добавили ли вы один и тот же сигнал несколько раз.

## Что мы использовали

- [Современный Python](../../field-guide/modern-python/) - виртуальная среда, типы и запуск проекта.
...
- Правила риска - небольшие функции и проверки, которые можно объяснить вручную.

## Усложняем проект

1. Добавьте белый список учебных доменов и отдельный сигнал для всех остальных.
