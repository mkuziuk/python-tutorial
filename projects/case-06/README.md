...
В стартовом файле есть только учебные ориентиры. Соберите программу по главе «Дело 06. Вердикт перед открытием»: готовая версия читает `data/evidence_bundle.json`, печатает отчёт и создаёт `verdict.json` рядом со скриптом.
