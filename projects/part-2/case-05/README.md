...
замороженного CSV-снимка Digits: сравните базовую модель, k-NN и RBF SVM на
одинаковых частях кросс-валидации, заранее объявите небольшую сетку параметров
...
Откройте папку расследования в системном приложении «Файлы» (Finder в macOS
или Проводник в Windows). Щёлкните по папке правой кнопкой мыши и выберите
команду открытия терминала в этой папке. Название команды зависит от системы:
например, «Новый Терминал по адресу папки» или «Открыть в терминале».
...
Тетрадь с решением создаёт `artifacts/model_lock.json` и
`artifacts/audit_memo.md`. В тетради для самостоятельной работы эти файлы
появятся после выполнения упражнения по фиксации результата. Блокировка
фиксирует модель до расследования II-06; внешнюю тестовую выборку нельзя
использовать для расширения сетки параметров. Перед сдачей выполните
