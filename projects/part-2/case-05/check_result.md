...
- Проверены форма и типы столбцов, диапазоны пикселей и цели, пропуски,
  полные дубликаты и распределение целевой переменной.
...
- Базовая модель `DummyClassifier`, k-NN(5) и RBF SVM сравнены на одинаковых
  частях по доле правильных ответов (accuracy) и macro-F1.
...
- До фиксации конфигурации журнал доступа к внешней тестовой выборке пуст;
  после неё записано ровно одно событие финальной оценки.
...
- Самый низкий recall у цифры 8 (примерно 0,930); самая частая направленная
  путаница — 8 → 1 (2 случая).
